...
async function loadSessionHistory(sessionId: number) {
  ...
  try {
    ...
    console.error('Load session history failed:', '')
    messages.value = []
  } finally {
...
async function handleNewSession() {
  try {
    ...
    console.error('Create session failed:', '创建会话失败')
  }
}
...
async function handleRenameSession(id: number, title: string) {
  try {
    ...
    console.error('Rename session failed:', '重命名失败')
  }
}
...
async function handleDeleteSession(id: number) {
  ...
  try {
    ...
    console.error('Delete session failed:', '删除失败')
  }
}
